...
mod tests {
    ...
    fn it_correctly_converts_to_string_for_select_tests() {
        let tests = [
            ...
            "SELECT memory_total_bytes, memory_swap_total_bytes - memory_swap_free_bytes AS swap, date_part($1, s.collected_at) AS collected_at FROM snapshots s INNER JOIN system_snapshots ON snapshot_id = s.id WHERE s.database_id = $2 AND s.collected_at >= $3 AND s.collected_at <= $4 ORDER BY collected_at ASC",
            ...
            "SELECT * FROM x WHERE x = ALL($1)",
            "SELECT * FROM x WHERE x = ANY($1)",
            "SELECT * FROM x WHERE x = COALESCE(y, $1)",
    ...
    // Merge support is not included yet
    // #[test]
    // fn it_correctly_converts_to_string_for_merge_tests() {
    //     let tests = [
    ...
    fn it_correctly_converts_to_string_for_create_function_tests() {
        let tests = [
            ...
            "CREATE OR REPLACE FUNCTION getfoo() RETURNS text AS $$SELECT name FROM \"users\" LIMIT 1$$ LANGUAGE sql IMMUTABLE SECURITY DEFINER",
            "CREATE OR REPLACE FUNCTION getfoo() RETURNS text AS $$SELECT name FROM \"users\" LIMIT 1$$ LANGUAGE sql IMMUTABLE SECURITY INVOKER",
            "CREATE OR REPLACE FUNCTION getfoo() RETURNS text AS $$SELECT name FROM \"users\" LIMIT 1$$ LANGUAGE sql IMMUTABLE LEAKPROOF",
            "CREATE OR REPLACE FUNCTION getfoo() RETURNS text AS $$SELECT name FROM \"users\" LIMIT 1$$ LANGUAGE sql IMMUTABLE NOT LEAKPROOF",
        ];
